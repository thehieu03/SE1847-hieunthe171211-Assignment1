...
public class filterbyprice extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        DAO dao = new DAO();
        if (x.equals("asc") || x.equals("desc")) {
            ArrayList<Product> listP = dao.getAllProductArrange(x);
            ArrayList<Categories> listC = dao.getAllCategories();
            request.setAttribute("x", x);
            request.setAttribute("listC", listC);
            request.setAttribute("listP", listP);
            request.getRequestDispatcher("home.jsp").forward(request, response);
        }else{
            ArrayList<Product> listP = dao.getTop5Product();
            ArrayList<Categories> listC = dao.getAllCategories();
            request.setAttribute("x", x);
            request.setAttribute("listC", listC);
            request.setAttribute("listP", listP);
